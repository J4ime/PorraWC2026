...
object KnockoutBracketGenerator {
    ...
    data class DieciseisavoSlot(
        val matchId: Int,
        val homeType: SlotTeam,
        val awayType: SlotTeam
    )
    ...
    val dieciseisavoSlots = listOf(
        ...
        DieciseisavoSlot(77, SlotTeam.GroupPosition(1, "I"), SlotTeam.ThirdParty("I", listOf("C","D","F","G","H"))),
        DieciseisavoSlot(78, SlotTeam.GroupPosition(2, "E"), SlotTeam.GroupPosition(2, "I")),
        DieciseisavoSlot(79, SlotTeam.GroupPosition(1, "A"), SlotTeam.ThirdParty("A", listOf("C","E","F","H","I"))),
        DieciseisavoSlot(80, SlotTeam.GroupPosition(1, "L"), SlotTeam.ThirdParty("L", listOf("E","H","I","J","K"))),
        DieciseisavoSlot(81, SlotTeam.GroupPosition(1, "D"), SlotTeam.ThirdParty("D", listOf("B","E","F","I","J"))),
        DieciseisavoSlot(82, SlotTeam.GroupPosition(1, "G"), SlotTeam.ThirdParty("G", listOf("A","E","H","I","J"))),
        ...
        DieciseisavoSlot(85, SlotTeam.GroupPosition(1, "B"), SlotTeam.ThirdParty("B", listOf("E","F","G","I","J"))),
        DieciseisavoSlot(86, SlotTeam.GroupPosition(1, "J"), SlotTeam.GroupPosition(2, "H")),
        DieciseisavoSlot(87, SlotTeam.GroupPosition(1, "K"), SlotTeam.ThirdParty("K", listOf("D","E","I","J","L"))),
        DieciseisavoSlot(88, SlotTeam.GroupPosition(2, "D"), SlotTeam.GroupPosition(2, "G"))
    )

    private val winnerGroupLetters = listOf("A", "B", "D", "E", "G", "I", "K", "L")

    private val thirdPlaceMatrix: Map<String, Map<String, String>> by lazy {
        val raw = matrixRaw.trimIndent().lines()
        raw.associate { line ->
            val parts = line.split("|")
            val key = parts[0]
            val assignments = winnerGroupLetters.zip(
                parts.drop(1)
            ).toMap()
            key to assignments
        }
    ...
    fun generateDieciseisavos(
        ...
        val topTwo = mutableMapOf<String, MutableMap<Int, String>>()
    ...
    private fun resolveTeam(
        ...
        return when (slot) {
            ...
            is SlotTeam.ThirdParty -> {
                assignedThird[slot.winnerGroup]
    ...
    private fun assignThirdPlaced(thirdTeams: List<ThirdPlaceTeam>): Map<String, String> {
        if (thirdTeams.size < 8) return emptyMap()
        val advancingGroups = thirdTeams.map { it.group }.sorted().joinToString("")
        val assignments = thirdPlaceMatrix[advancingGroups] ?: return emptyMap()

        val result = mutableMapOf<String, String>()
        for ((winnerGroup, thirdGroup) in assignments) {
            val team = thirdTeams.firstOrNull { it.group == thirdGroup }?.teamName ?: continue
            result[winnerGroup] = team
        }
        return result
    }
